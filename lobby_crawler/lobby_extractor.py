            ...
            if lobbyEntryDetails["lobbyistIdentity"]["identity"]  == "ORGANIZATION":
                ...
                try:
                    if lobbyEntryDetails["refuseFinancialExpensesInformation"]:
                        ...
                        lobby.lobbyMoneySpent.start = 0
                        lobby.lobbyMoneySpent.to = 0
                        lobby.lobbyMoneySpent.year = ""
                        lobby.lobbyMoneySpent.reason = lobbyEntryDetails["refuseFinancialExpensesInformationReason"]
                    else:
                        financialExpenses = lobbyEntryDetails["financialExpensesEuro"]
                        lobby.lobbyMoneySpent.start = financialExpenses["from"]
                        lobby.lobbyMoneySpent.to = financialExpenses["to"]
                        yearString = financialExpenses["fiscalYearStart"]
                        lobby.lobbyMoneySpent.year = "20"+yearString[yearString.find("/")+1:]
                        lobby.lobbyMoneySpent.reason = ""
                except:
                    log.debug("no Finance Info")
                    lobby.lobbyMoneySpent.start = 0
                    lobby.lobbyMoneySpent.to = 0
